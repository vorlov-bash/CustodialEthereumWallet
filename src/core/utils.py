...
def build_openapi_responses(list_of_errors: list[JsonException]) -> dict:
    mapping = {}
    for error in list_of_errors:
        mapping.setdefault(error.status_code, []).append(error)
    responses = {}

    for status_code, errors in mapping.items():
        if len(errors) == 1 or isinstance(errors, JsonException):
            error = errors[0] if isinstance(errors, list) else errors

            responses[status_code] = {
                "content": {"application/json": {"example": error.raw_response()}}
            }
        else:
            responses[status_code] = {
                "content": {
...
def signal_fence(
    ...
    *,
    on_deferred_signal: Callable[[int, FrameType | None], None] = None,
) -> Iterator[None]:  # noqa: UP007 E722
    """
